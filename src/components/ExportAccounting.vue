    ...
    async exportExcel(row) {
      ...
      try {
        ...
        const response = await fetch(
          new URL("@/assets/傳票範本.xlsx", import.meta.url).href
        );
        ...
        worksheet.getCell(`G${totalAmountRow}`).value = parseFloat(row.amount); // 轉數值
        worksheet.getCell(`G${totalAmountRow}`).numFmt = "#,##0"; // 千分位格式
        worksheet.getCell(`H${totalAmountRow}`).value = parseFloat(row.amount); // 轉數值
        ...
        link.download = `${row.id}傳票.xlsx`; // 設定下載檔案名
        link.click();
        console.log("結束");
      } catch (error) {
