<template>
  ...
  <el-form-item label="中油未回傳車輛" class="section-header">
    <el-table :data="paginatedDiscount" style="width: 100%" v-loading="loading">
      ...
      <el-table-column prop="vehicleId" label="車號" width="300"
        ><template v-slot="scope"
          >{{ formatName(scope.row.vehicleId) }}
        </template></el-table-column
      >
      <el-table-column
        prop="card_type"
        label="卡片類別"
        :formatter="product_method"
        width="150"
      />
...
export default {
  ...
  methods: {
    ...
    async getdata() {
      await axios
        .get("/apiServer/main/selectCPCdata")
        .then((response) => {
    ...
    async submitData() {
      ...
      await axios
        .post("/apiServer/main/importCPCfile", jsonData)
        .then((response) => {
