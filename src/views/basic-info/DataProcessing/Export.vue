<template>
  ...
  <el-select
    v-model="this.cpc_account"
    placeholder="選擇中油帳號"
    style="margin-right: 20px"
    @change="filterRecorded"
  >
  ...
  >
    <el-form :model="form" label-width="120px">
      ...
      <el-row style="margin-bottom: 20px">
        <el-form-item
          label="*卡號"
          v-if="this.form.state !== 1 && this.form.state !== 7"
        >
        ...
        <el-form-item label="*上傳中油原因">
          ...
          >
            ...
            <el-option
              v-if="form.state == 1 || form.state == 7"
              label="新增"
              :value="'新增'"
            ></el-option>
            <!-- 其他選項固定顯示 -->
            ...
            ></el-option>
            <el-option
              v-if="form.state == 3 || form.state == 6"
              label="改客戶(原卡號停用)"
              :value="'停用'"
            ></el-option>
            <el-option
...
export default {
  ...
  methods: {
    ...
    filterRecorded() {
      if (this.cpc_account) {
        this.filteredRecorded = this.Recorded.filter(
          (item) => item.cpc_account == this.cpc_account
        );
      } else {
        // 如果沒有選擇特定的帳號，顯示所有資料
    ...
    async getVehicle() {
      ...
      if (
        this.form.license_plate.length >= 5 &&
        this.form.license_plate.length <= 11
      ) {
        try {
          //1.先找全部車牌 有就判斷 沒就新增
          this.selectcards=[]
