...
export default {
  ...
  methods: {
    async getbillselectData() {
      ...
      await axios.post('/apiServer/main/searchAccount_sort',postData)
        .then(response => {
    ...
    savePass() {
      const req = this.form;
      // 發送 POST 請求
      axios.post('/apiServer/main/createCard', req)
        .then(response => {
    ...
    async getselectData() {
      ...
      await axios.post('/apiServer/main/searchCard',postData)
        .then(response => {
    ...
    async deleteItem(row) {
      ...
      await axios.post('/apiServer/main/deleteCard', req)
        .then(response => {
          if (response.status === 200 && response.data.returnCode === 0) {
            // 成功提示
            this.$message({
